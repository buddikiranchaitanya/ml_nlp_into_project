...
df_tokens_train = pand.read_csv("NELA_Data/NELA_Train.csv")
df_tokens_test = pand.read_csv("NELA_Data/NELA_Test.csv")
df_tokens_train = df_tokens_train.sample(frac = 0.6) 
...
df_tokens_train['ID'] = df_tokens_train['ID'].astype(str).astype(int) 
df_tokens_train['Label'] = df_tokens_train['Label'].astype(str).astype(int) 
df_tokens_test['ID'] = df_tokens_test['ID'].astype(str).astype(int) 
df_tokens_test['Label'] = df_tokens_test['Label'].astype(str).astype(int) 


# In[5]:


labels_test=df_tokens_test['Label']
labels_train= df_tokens_train['Label']
...
tokenizer.fit_on_texts(df_tokens_train['Headline'].append(df_tokens_train['Body']))
...
train_body_features_unpad = tokenizer.texts_to_sequences(df_tokens_train['Body'])
...
test_body_features_unpad = tokenizer.texts_to_sequences(df_tokens_test['Body'])
...
def evaluate_classifiers (X_train, y_train, X_test, y_test, dim_red, feature_selection):
    ...
    ploty.figure()
    ConfusionMatrixDisplay.from_predictions(actual, predicted, cmap='PuRd')
    ploty.show()
    ...
    ax2 = ax.twinx()
    ax.set_ylabel('Fit times', color='#2ca02c')
    ax2.set_ylabel('Accuracy Scores', color='#ff7f0e')
    x=nmp.arange(4)
    X=['Decision Tree', 'Random Forest', 'KNeighbors', 'SGDC']
    ax.bar(x-0.1, [DT_fit_time, RFC_fit_time, KN_fit_time, SGDC_fit_time],0.2, color=['cyan'], label='Fit times')
    ax2.bar(x+0.1, [accuracy_DT, accuracy_RFC, accuracy_KN, accuracy_SGDC], 0.2,  color=['olive'], label='Accuracy scores')
    ax2.set_ylim([0, 1])
